...
def _is_type(symbol, language, cb, cwd='.', timeout=99):
    ret = []
    ts_init(language)

    # ripgrep after the symbol
    results = rg(f"\W{symbol}([\W$])", cwd=cwd, timeout=timeout)
    ...
    for file in results:
        ...
        tree = ts_parse_file(language, file_path)
        if tree == None:
...
def goto_definition(symbol, language, cwd='.', timeout=99):
    results = _is_type(symbol, language, ts_is_definition, cwd=cwd, timeout=timeout)
    return results
